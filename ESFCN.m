function bestfis=ESFCN(fis,data)
% Variables
...
results=RunESFCN(Problem,Params);
% Getting the Results
p=results.BestSol.Position.*p0;
bestfis=FuzzyParameters(fis,p);
end
%%----------------------------------------------
function results=RunESFCN(Problem,Params)
...
% Plot Evolution Strategy Algorithm Training Stages
...
xlabel('ES Iteration Number','FontSize',8,...
'FontWeight','bold','Color','b');
ylabel('ES Best Cost Result','FontSize',8,...
'FontWeight','bold','Color','b');
legend({'Evolution Strategy Train'});
end
